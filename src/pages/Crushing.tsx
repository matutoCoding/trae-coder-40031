...
const tabs = [
  { label: '冷却破碎', icon: Package },
  { label: '发气量检测', icon: Thermometer },
  { label: '入库分级', icon: Archive },
];
...
export default function Crushing() {
  ...
  const barData = crushings.map(c => ({ name: c.batchNo.slice(-6), 破碎量: c.crushedWeight }));
  const lineData = gasTests.map(g => ({ name: g.batchNo.slice(-6), 发气量: g.gasVolume }));
  const pieData = Object.entries(
    storages.reduce<Record<string, number>>((acc, s) => {
      acc[s.grade] = (acc[s.grade] || 0) + s.quantity;
      return acc;
    }, {})
    ...
    <div className="space-y-6">
      <div className="flex gap-2">
        {tabs.map((t, i) => (
          <button
            key={i}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${tab === i ? 'bg-lava-500/20 text-lava-400 border border-lava-500/30' : 'bg-furnace-800/50 text-steel-400 border border-transparent hover:text-steel-200'}`}
            onClick={() => setTab(i)}
          >
      ...
      {tab === 0 && (
        <>
          <div className="flex items-center justify-between">
            ...
            <button className="btn-primary flex items-center gap-2" onClick={() => setShowCrushForm(!showCrushForm)}>
              <Plus className="w-4 h-4" />新增破碎记录
            </button>
          </div>
          {showCrushForm && (
            <div className="card-glow p-4 flex flex-wrap items-end gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">批次号</label>
                <input className="input-field w-40" value="自动生成" disabled />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">冷却时长(h)</label>
                <input className="input-field w-24" type="number" value={crushForm.coolingHours} onChange={e => setCrushForm(f => ({ ...f, coolingHours: +e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">粒度</label>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">破碎量(kg)</label>
                <input className="input-field w-28" type="number" value={crushForm.crushedWeight} onChange={e => setCrushForm(f => ({ ...f, crushedWeight: +e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">操作人</label>
                <input className="input-field w-28" value={crushForm.operator} onChange={e => setCrushForm(f => ({ ...f, operator: e.target.value }))} />
              </div>
      ...
      {tab === 1 && (
        <>
          <div className="flex items-center justify-between">
            ...
            <button className="btn-primary flex items-center gap-2" onClick={() => setShowGasForm(!showGasForm)}>
              <Plus className="w-4 h-4" />新增检测记录
            </button>
          </div>
          {showGasForm && (
            <div className="card-glow p-4 flex flex-wrap items-end gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">发气量(L/kg)</label>
                <input className="input-field w-28" type="number" value={gasForm.gasVolume} onChange={e => setGasForm(f => ({ ...f, gasVolume: +e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400 flex items-center gap-2">
                  <input type="checkbox" checked={gasForm.isQualified} onChange={e => setGasForm(f => ({ ...f, isQualified: e.target.checked }))} />
                  是否合格
                </label>
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">检测人</label>
                <input className="input-field w-28" value={gasForm.tester} onChange={e => setGasForm(f => ({ ...f, tester: e.target.value }))} />
              </div>
      ...
      {tab === 2 && (
        <>
          <div className="flex items-center justify-between">
            ...
            <button className="btn-primary flex items-center gap-2" onClick={() => setShowStoreForm(!showStoreForm)}>
              <Plus className="w-4 h-4" />新增入库记录
            </button>
          </div>
          {showStoreForm && (
            <div className="card-glow p-4 flex flex-wrap items-end gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">批次号</label>
                <input className="input-field w-44" value={storeForm.batchNo} onChange={e => setStoreForm(f => ({ ...f, batchNo: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">等级</label>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">数量(kg)</label>
                <input className="input-field w-28" type="number" value={storeForm.quantity} onChange={e => setStoreForm(f => ({ ...f, quantity: +e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">仓库</label>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-steel-400">操作人</label>
                <input className="input-field w-28" value={storeForm.operator} onChange={e => setStoreForm(f => ({ ...f, operator: e.target.value }))} />
              </div>
            ...
            <div className="card-glow p-5">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  ...
                  <Legend wrapperStyle={{ fontSize: '12px' }} formatter={(v: string) => <span className="text-steel-300">{v}</span>} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </div>
