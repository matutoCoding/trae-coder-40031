...
export default function Feeding() {
  ...
  const filteredFeedings = useMemo(() => filterByShiftAndDate(feedings, shiftFilter, dateRange), [feedings, shiftFilter, dateRange]);
  ...
  const chartData = useMemo(() => {
    ...
    filteredFeedings.forEach(f => {
      ...
      f.material === 'lime' ? entry.lime += f.weight : entry.coke += f.weight;
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="btn-primary flex items-center gap-2" onClick={() => setShowForm(true)}><Plus className="w-4 h-4" />新增上料</button>
      </div>
          ...
          <tbody>
            {filteredFeedings.map(f => (
              <tr key={f.id}>
                ...
                <td>{f.siloNo}</td>
                <td>{f.material === 'lime' ? <span className="badge-info">石灰</span> : <span className="badge-warning">焦炭</span>}</td>
            ...
            {silos.map(silo => (
              <div key={silo.label} className="flex flex-col items-center gap-3 flex-1">
                ...
                <div className="w-20 h-48 bg-furnace-800 rounded-lg border border-furnace-500/30 relative overflow-hidden">
                  <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t ${silo.gradient} transition-all duration-700 rounded-b-lg`} style={{ height: `${silo.level}%` }} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="card-glow w-full max-w-md p-6 mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button className="text-steel-500 hover:text-steel-300 transition-colors" onClick={() => setShowForm(false)}><X className="w-5 h-5" /></button>
            </div>
